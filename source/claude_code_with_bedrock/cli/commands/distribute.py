...
class DistributeCommand(Command):
    ...
    def _create_archive(self, package_path: Path) -> Path:
        ...
        required_files = [
            ...
            "config.json",
            "README.md",
            # Claude Desktop (CoWork 3P) MDM configs — generated by `ccwb package`.
            # shutil.copy2 below is gated on source.exists(), so missing files
            # (e.g. when packaging failed before MDM generation) are harmless.
