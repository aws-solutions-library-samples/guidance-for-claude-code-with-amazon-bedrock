...
class MultiProviderAuth:
    ...
    def clear_cached_credentials(self):
        ...
        # Clear credentials file — session storage mode only.
        # In keyring mode the shared credentials file is not the source of truth, and
        # boto3 resolves ~/.aws/credentials BEFORE credential_process in ~/.aws/config.
        # An EXPIRED stanza here would shadow the real credential_process line and
        # break Claude Desktop's inferenceBedrockProfile lookup with a 403 forever.
        if self.credential_storage == "session":
            try:
    ...
    def _update_claude_otel_attributes(self, attributes):
        ...
        tmp_fd, tmp_path = tempfile.mkstemp(dir=cache_dir, prefix=".otel-identity-tmp-")
        try:
        ...
        # Migration: strip any stale OTEL_RESOURCE_ATTRIBUTES from settings.json
        # (self-healing for users who had it set by earlier installs).
        # Runs on re-auth and at install so no separate cleanup script is needed; idempotent.
        if os.name != "nt":
            settings_path = Path.home() / ".claude" / "settings.json"
            if settings_path.exists():
                try:
                    with open(settings_path, "r") as f:
                        settings = json.load(f)
                    if settings.get("env", {}).pop("OTEL_RESOURCE_ATTRIBUTES", None) is not None:
                        with open(settings_path, "w") as f:
                            json.dump(settings, f, indent=2)
                except Exception:
