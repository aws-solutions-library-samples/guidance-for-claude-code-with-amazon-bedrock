...
When a monitoring stack is deployed, `ccwb cowork generate` automatically configures CoWork telemetry routing. The telemetry path depends on the monitoring mode:

### Central mode (default)

```
CoWork → localhost:4318 (otel-helper proxy) → Remote ALB → ECS Collector → CloudWatch
```

The local otel-helper proxy is **auto-spawned by credential-process** on every invocation. It:
1. Reads the user's decoded JWT identity from the local cache (`~/.claude-code-session/{profile}-otel-headers.json`)
2. Injects `x-user-email`, `x-department`, `x-team-id`, etc. headers into OTLP requests
3. Forwards to the remote collector ALB

This enables **per-user identity** for CoWork telemetry — dashboards show individual user attribution, not just device-level data.

### Sidecar mode

```
CoWork → localhost:4319 (otel-helper proxy) → localhost:4318 (otelcol) → CloudWatch
```

In sidecar mode, the OpenTelemetry Collector (`otelcol`) already occupies port 4318. The identity proxy listens on **port 4319** and chains through to otelcol:
1. Proxy injects per-user identity headers (same as central mode)
2. Forwards to local otelcol on port 4318
3. otelcol reads identity from metadata (`include_metadata: true`), SigV4-signs, exports to CloudWatch

### Proxy lifecycle (self-healing)

The proxy is managed automatically — no LaunchAgent, systemd service, or manual startup required:

1. User opens Claude Desktop → Cowork needs Bedrock credentials
2. AWS SDK calls `credential-process` → auth succeeds → identity cache populated
3. `credential-process` checks if proxy is listening (TCP port check, 200ms timeout)
4. If not listening → spawns `otel-helper --proxy <upstream> --proxy-port <port>` as a detached process
5. Proxy serves until killed or machine restarts
6. If proxy dies → next `credential-process` invocation restarts it automatically

> **Refresh cadence by auth flow:**
>
> | Auth Flow | How often credential-process runs | Proxy recovery time |
> |-----------|-----------------------------------|---------------------|
> | **OIDC** (Cognito/Okta/Azure) | Every SDK call when cached creds expire (~1h with Cognito, up to 8h with direct STS) | Every invocation checks liveness |
> | **IDC** (Identity Center) | Every SDK call (SSO token cache managed externally by `aws sso login`) | Every invocation checks liveness |
> | **Passthrough** (ambient creds) | Every SDK call (no internal caching) | Every invocation checks liveness |
>
> With the liveness check on every cached return path, proxy recovery is immediate on the next Bedrock API call — not gated by credential expiry.

### No proxy required?

If the monitoring stack is not deployed (no `otel_collector_endpoint` in the profile), CoWork telemetry is silently dropped — no proxy is spawned and `otlpEndpoint` is not set in the MDM config.
...
CoWork telemetry now supports **per-user identity** via the local otel-helper proxy. The proxy reads the user's decoded JWT from the identity cache and injects these headers into every OTLP request:

| Header | Source | Description |
|--------|--------|-------------|
| `x-user-email` | JWT `email` claim | Primary user identifier |
| `x-user-id` | JWT `sub` claim | Stable user ID |
| `x-user-name` | JWT `preferred_username` | Display name |
| `x-department` | JWT `department` | Org department |
| `x-team-id` | JWT `team`/`groups` | Team identifier |
| `x-project` | Custom tag key from config | Cost attribution tag |

These headers flow through the collector pipeline and appear as CloudWatch dimensions, enabling per-user dashboards and cost attribution.

> **How it works:** The identity cache is populated by `credential-process` every time it decodes the user's OIDC token (on every credential refresh). The proxy reads this cache on each OTLP request and injects the headers.

> **IDC mode:** IAM Identity Center does not provide a JWT with user claims. In IDC deployments, CoWork telemetry is device-level only (using the anonymous `user.id` UUID from Claude Desktop).

### Device-level fallback

Every CoWork 3P OTEL event also includes a `user.id` attribute — an anonymous device/installation UUID generated by Claude Desktop. This is always present regardless of proxy configuration and can be used for:

- Per-device cost attribution when user identity is unavailable
