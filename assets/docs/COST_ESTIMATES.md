...
This is the part that scales with your team. The estimates below assume a **heavy / power-user** pattern of roughly **2M input + 500K output tokens per developer per working day**, over **22 working days/month**. Lighter usage costs proportionally less (see note).

On-demand pricing, US East (N. Virginia):

| Model | Input ($/1M) | Output ($/1M) | Cache read ($/1M) | Est. per user/day | Est. per user/month |
|---|---|---|---|---|---|
| **Claude Haiku 4.5** | $1.00 | $5.00 | $0.10 | ~$4.50 | **~$99** |
| **Claude Sonnet 4.5 / 4.6** | $3.00 | $15.00 | $0.30 | ~$13.50 | **~$297** |
| **Claude Opus 4.5 / 4.6 / 4.7 / 4.8** | $5.00 | $25.00 | $0.50 | ~$22.50 | **~$495** |

> **Usage varies widely.** A **light** user (~500K input + 100K output/day) running Sonnet costs about **$3/day (~$66/month)** — less than a quarter of the heavy-user figure. The numbers above represent heavy coding usage. Treat them as an upper-ish bound for typical developers.
...
Combining fixed infrastructure with variable Bedrock usage. Assumes **heavy usage on Claude Sonnet** (~$297/user/month) unless noted, and **Sidecar monitoring** (near-zero infra) vs. **Central monitoring** (~$45 infra). Bedrock usage is the same in both columns — only the infrastructure differs.

| Team size | Bedrock (Sonnet, heavy) | + Sidecar monitoring | + Central monitoring | Notes |
|---|---|---|---|---|
| **10 users** | ~$2,970 | ~$2,985 | ~$3,015 | Pilot team |
| **25 users** | ~$7,425 | ~$7,440 | ~$7,470 | Department |
| **50 users** | ~$14,850 | ~$14,865 | ~$14,895 | Division |
| **100 users** | ~$29,700 | ~$29,715 | ~$29,745 | Enterprise |
| **50 users (Haiku)** | ~$4,950 | ~$4,965 | ~$4,995 | Cost-optimized model choice |

The columns are nearly identical because infrastructure is negligible relative to token usage. **Model choice and prompt caching move the number far more than any infrastructure decision.** For example, the same 50-user team costs ~$14,850/month on Sonnet but ~$4,950/month on Haiku — a ~$9,900/month difference, versus the ~$45/month spread between monitoring modes.
