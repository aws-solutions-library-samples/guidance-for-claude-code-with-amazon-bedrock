...
### CoWork 3P
- MDM config generated by `ccwb package` (`cowork_3p.py`)
- Service token auth (not user JWT) for telemetry ingest
- Telemetry schema differs from Claude Code — test both paths when modifying collector/dashboards
- Extra MDM keys via profile JSON editing (not init wizard)
