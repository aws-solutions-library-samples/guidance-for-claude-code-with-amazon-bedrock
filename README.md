...
> **Note**: For AWS GovCloud deployment instructions, see [AWS Partition Support](#aws-partition-support) section.
...
# Check Bedrock policy uses correct partition ARNs
aws iam list-attached-role-policies \
  --role-name BedrockCognitoFederatedRole \
  --region <region> \
  --query 'AttachedPolicies[0].PolicyArn' \
  --output text | xargs -I {} aws iam get-policy-version \
  --policy-arn {} \
  --version-id v1 \
  --query 'PolicyVersion.Document.Statement[0].Resource'

# Expected Bedrock resource ARNs:
# Commercial: ["arn:aws:bedrock:*::foundation-model/*", ...]
# GovCloud: ["arn:aws-us-gov:bedrock:*::foundation-model/*", ...]
